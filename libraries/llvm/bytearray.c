...
struct Pos c_bytearray_new(const Int size)
{
	...
	*headerPtr = (struct Header){
		.rc = 0,
		.eraser = c_bytearray_erase_noop,
	};
	return (struct Pos){
		.tag = size,
		.obj = objPtr,
...
struct Pos c_bytearray_set(const struct Pos arr, const Int index,
			   const Byte value)
{
	Byte *dataPtr = arr.obj + sizeof(struct Header);
	dataPtr[index] = value;
...
// TODO do this in Effekt
struct Pos c_bytearray_concatenate(const struct Pos left,
				   const struct Pos right)
{
	...
	for (uint64_t j = 0; j < concatenated.tag; ++j)
		c_bytearray_data(concatenated)[j] =
			j < left_size ? c_bytearray_data(left)[j] :
...
struct Pos c_bytearray_equal(const struct Pos left, const struct Pos right)
{
	...
	int cmp = memcmp(c_bytearray_data(left), c_bytearray_data(right),
			 left_size);
	erasePositive(left);
	erasePositive(right);
...
// TODO deprecate
struct Pos c_bytearray_substring(const struct Pos str, uint64_t start,
				 uint64_t end)
{
